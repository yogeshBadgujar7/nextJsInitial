   ...
   */ async writeAutomaticFontOptimizationManifest() {
    ...
    async writeBuildManifest(pageEntrypoints, rewrites) {
        const processedRewrites = {
            ...rewrites,
            beforeFiles: ((rewrites == null ? void 0 : rewrites.beforeFiles) ?? []).map(_buildmanifestplugin.processRoute),
            afterFiles: ((rewrites == null ? void 0 : rewrites.afterFiles) ?? []).map(_buildmanifestplugin.processRoute),
            fallback: ((rewrites == null ? void 0 : rewrites.fallback) ?? []).map(_buildmanifestplugin.processRoute)
        };
        ...
        const interceptionRewrites = JSON.stringify(processedRewrites.beforeFiles.filter(_generateinterceptionroutesrewrites.isInterceptionRouteRewrite));
        ...
        const content = {
            __rewrites: (0, _buildmanifestplugin.normalizeRewritesForBuildManifest)(processedRewrites),
            ...Object.fromEntries([
