   ...
   */ async writeAutomaticFontOptimizationManifest() {
    ...
    async writeBuildManifest(pageEntrypoints, rewrites) {
        const processedRewrites = {
            ...rewrites,
            beforeFiles: ((rewrites == null ? void 0 : rewrites.beforeFiles) ?? []).map(processRoute),
            afterFiles: ((rewrites == null ? void 0 : rewrites.afterFiles) ?? []).map(processRoute),
            fallback: ((rewrites == null ? void 0 : rewrites.fallback) ?? []).map(processRoute)
        };
        ...
        const interceptionRewrites = JSON.stringify(processedRewrites.beforeFiles.filter(isInterceptionRouteRewrite));
        ...
        const content = {
            __rewrites: normalizeRewritesForBuildManifest(processedRewrites),
            ...Object.fromEntries([
