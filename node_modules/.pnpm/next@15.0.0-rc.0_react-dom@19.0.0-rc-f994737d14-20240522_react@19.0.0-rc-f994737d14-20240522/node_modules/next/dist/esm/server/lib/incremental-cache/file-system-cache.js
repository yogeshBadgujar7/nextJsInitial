...
export default class FileSystemCache {
   ...
   */ async loadTagsManifest() {
        ...
        try {
            tagsManifest = JSON.parse(await this.fs.readFile(this.tagsManifestPath, 'utf8'));
        } catch (err) {
            tagsManifest = {
                version: 1,
                items: {}
   ...
   */ loadTagsManifestSync() {
        ...
        if (!data && process.env.NEXT_RUNTIME !== 'edge') {
            ...
            try {
                ...
                if (kind === 'fetch' && this.flushToDisk) {
                    ...
                    const pageData = isAppPath ? await this.fs.readFile(this.getFilePath(`${key}${this.isAppPPREnabled ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX}`, 'app'), 'utf8') : JSON.parse(await this.fs.readFile(this.getFilePath(`${key}${NEXT_DATA_SUFFIX}`, 'pages'), 'utf8'));
                    let meta;
                    if (isAppPath) {
                        try {
    ...
    async set(...args) {
        ...
        if ((data == null ? void 0 : data.kind) === 'PAGE') {
            ...
            await this.fs.writeFile(this.getFilePath(`${key}${isAppPath ? this.isAppPPREnabled ? RSC_PREFETCH_SUFFIX : RSC_SUFFIX : NEXT_DATA_SUFFIX}`, isAppPath ? 'app' : 'pages'), isAppPath ? data.pageData : JSON.stringify(data.pageData));
